...
const CategoryItem = ({ category, level = 0, onAddSub, onEdit, onDelete }) => {
    const [isExpanded, setIsExpanded] = useState(true);

    return (
        <div className="mb-2">
            <div className={`flex items-center justify-between p-3 bg-white border border-gray-200 rounded-lg hover:border-primary-300 transition group ${level > 0 ? 'ml-8' : ''}`}>
                <div className="flex items-center flex-1">
                    ...
                    {!category.children?.length && level < 2 && <div className="w-6" />}
...
export default function Categories() {
    ...
    const handleOpenModal = (parent = null, editing = null) => {
        setParentCategory(parent);
        setEditingCategory(editing);
        setFormData({
            name: editing ? editing.name : '',
            is_active: editing ? editing.is_active : true,
            parent_id: parent ? parent.id : (editing ? editing.parent_id : null)
        });
        setIsModalOpen(true);
    };
            ...
            <main className="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
                ...
                {isLoading ? (
                    ...
                    <div className="bg-white rounded-xl border border-gray-200 p-12 text-center text-gray-500">
                        No categories defined yet. Start by adding a root category.
                    </div>
                ) : (
                    <div className="space-y-4">
                        {categories.map(cat => (
                            <CategoryItem
                                key={cat.id}
                                category={cat}
                                onAddSub={(p) => handleOpenModal(p)}
                                onEdit={(c) => handleOpenModal(null, c)}
                    ...
                    <div className="bg-white rounded-xl shadow-xl max-w-md w-full overflow-hidden">
                        <div className="p-6 border-b border-gray-100 bg-gray-50 flex justify-between items-center">
                            <h2 className="text-xl font-bold text-gray-900">
                                {editingCategory ? 'Edit Category' : (parentCategory ? `Add to ${parentCategory.name}` : 'Add Root Category')}
                            </h2>
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-4">
                            ...
                            <div className="flex items-center">
                                <label className="flex items-center cursor-pointer">
                                    ...
                                    />
                                    <span className="ml-2 text-sm text-gray-700">Active and visible to agents</span>
                                </label>
                            </div>
